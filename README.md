...
$ cd eb-cosmo
```
checkout the eb_generator branch:
```
$ git checkout eb_generator 
```
...
```
where `-p` is the project (or setup): `crclim` or `cordex`, if you wish to build your personalized stella/dycore put the name of the project here.
The `-t` is the target: `cpu` or `gpu`, and `-i` the module install path.
If you are building a personalized version make use of the `-k ksize` and `-f kflat` option. In this case you will also need to invoke the `-q force proj` option.
The `-z` option launches a clean build.
...
```
or a version with 74 vertical levels and kflat of 44 for GPU in `/scratch/username/install/` with the project name "rce":
```
...
As we previously installed the Cordex Dycore for GPU, replace the `Options.lib.gpu` file with the file generated by the build_crclim_libs.sh:
```
cosmo-pompa/cosmo/$ cp /scratch/username/eb-cosmo/Options.lib.gpu .
```
If you compile another version of Cosmo targeting the CPU, you need to copy the `Options.lib.cpu` file.
The extended tutorial provides more details about these files and variables.

Load all corresponding modules (e.g. for a cordex setup on gpus):  
```
cosmo-pompa/cosmo/$ module load STELLA_CORDEX/cordex-CrayGNU-18.08-double 
cosmo-pompa/cosmo/$ DYCORE_CORDEX_GPU/cordex-CrayGNU-18.08-double
```
or using the custom "rce" configuration described above
```
module load STELLA_RCE/rce-CrayGNU-18.08-double
module load DYCORE_RCE_GPU/rce-CrayGNU-18.08-double
```
and run the good old Cosmo build script:
```
cosmo-pompa/cosmo/$ test/jenkins/./build.sh -t gpu -x $EBROOTDYCORE_CRCLIM_GPU
```
